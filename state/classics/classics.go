...
func (s *Classics) Next(player *service.Player) (consts.StateID, error) {
...
func (*Classics) Exit(player *service.Player) consts.StateID {
...
func handlePlay(player *service.Player, game *service.Game) error {
	...
	for {
		...
		if !master && len(game.LastPokers) > 0 {
			...
			buf.WriteString(fmt.Sprintf("Last player is %s: %s, sells: %s\n", flag, service.GetPlayer(game.LastPlayer).Name, game.LastPokers.String()))
		}
